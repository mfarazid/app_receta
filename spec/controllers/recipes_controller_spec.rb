...
describe RecipesController do
  ...
  describe "show" do
    ...
    context "when the recipe exists" do
      before do
        get :show, xhr: true, format: :json, params: {id: recipe[:id]} 
      end
      let(:recipe) { 
        Recipe.create!(name: 'Baked Potato w/ Cheese', 
    ...
    context "when the recipe doesn't exit" do
      before do
        get :show, xhr: true, format: :json, params: {id: -9999}
      end
  ...
  describe "create" do
    before do
      post :create, xhr: true, format: :json, params: {recipe: { name: "Toast", 
  ...
  describe "update" do
    ...
    before do
      put :update, xhr: true,format: :json, params: {id: recipe.id, recipe: { name: "Toast", 
                                                 instructions: "Add bread to toaster, push lever" }}
      recipe.reload
    end
  ...
  describe "destroy" do
    ...
    before do
      delete :destroy, format: :json, params: {id: recipe_id}
    end
    it { expect(response.status).to eq(204) }
